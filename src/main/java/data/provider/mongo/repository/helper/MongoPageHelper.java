package data.provider.mongo.repository.helper;
...
public class MongoPageHelper {
    ...
    public long countNum(Query query, Class entityClass){
     ...
     * Class, Function, Integer, Integer,String
     * String)
     */
...
//        Sort sort = new Sort(Sort.Direction.DESC, "version");
            final List<T> entityList = mongoTemplate.find(query.addCriteria(criteria)
                    .with(new Sort(Direction.DESC, "createTime")),entityClass);
            // .with( new Sort(Direction.DESC, "id")),entityClass);
            pageResult.setLists(entityList.stream().map(mapper).collect(Collectors.toList()));
        }
        return pageResult;
    }
